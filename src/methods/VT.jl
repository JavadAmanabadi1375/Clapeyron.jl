...
function equivol_cross_second_virial(model,T,p_exp = 200000.0)
    ...
    v01,v02 = _virial(B11,p_exp,T),volume_virial(B22,p_exp,T)
...
function VT_Gathering_Derivatives(model::EoSModel, V, T, z=SA[1.])
    ...
    dA_res, A_res = ∂f_res(model,V,T,z)
    ∂A∂V_res, ∂A∂T_res = dA_res
    return (grad_p[1],grad_p[2],∂²A∂V∂T,∂²A∂V²,∂²A∂T²,∂A∂V,∂A∂T,A,A_res,∂A∂T_res,∂A∂V_res)
end

function VT_res(model::EoSModel, V, T, z=SA[1.])
    d²A = f_hess_res(model,V,T,z)
    ∂²A∂V∂T = d²A[1,2]
    ∂²A∂V² = d²A[1,1]
    ∂²A∂T² = d²A[2,2]
...
function VT_div_hs(model::EoSModel, V, T, z=SA[1.])
    d²A = f_hess_res_hs(model,V,T,z)
    ∂²A∂V∂T = d²A[1,2]
    ∂²A∂V² = d²A[1,1]
    ∂²A∂T² = d²A[2,2]
    dAres, Ares = ∂f_res_hs(model,V,T,z)
    ∂A∂V, ∂A∂T = dAres
    return (∂²A∂V∂T,∂²A∂V²,∂²A∂T²,Ares,∂A∂T,∂A∂V)

end
function VT_div_chain(model::EoSModel, V, T, z=SA[1.])
    d²A = f_hess_res_chain(model,V,T,z)
    ∂²A∂V∂T = d²A[1,2]
    ∂²A∂V² = d²A[1,1]
    ∂²A∂T² = d²A[2,2]
    dAres, Ares = ∂f_res_chain(model,V,T,z)
    ∂A∂V, ∂A∂T = dAres
    return (∂²A∂V∂T,∂²A∂V²,∂²A∂T²,Ares,∂A∂T,∂A∂V)


end
function VT_div_disp(model::EoSModel, V, T, z=SA[1.])
    d²A = f_hess_res_disp(model,V,T,z)
    ∂²A∂V∂T = d²A[1,2]
    ∂²A∂V² = d²A[1,1]
    ∂²A∂T² = d²A[2,2]
    dAres, Ares = ∂f_res_disp(model,V,T,z)
    ∂A∂V, ∂A∂T = dAres
    return (∂²A∂V∂T,∂²A∂V²,∂²A∂T²,Ares,∂A∂T,∂A∂V)
