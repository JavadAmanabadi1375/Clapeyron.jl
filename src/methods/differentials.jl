...
"""
∂f_res_hs(model,V,T,z)
∂f_res_hc(model,V,T,z)
∂f_res_disp(model,V,T,z)

"""
function ∂f_res_hs(model,V,T,z)
    f(∂V,∂T) = eos_res_hs(model,∂V,∂T,z)
    _f,_df = Solvers.fgradf2(f,V,T)
    return _df,_f
end
function ∂f_res_chain(model,V,T,z)
    f(∂V,∂T) = eos_res_chain(model,∂V,∂T,z)
    _f,_df = Solvers.fgradf2(f,V,T)
    return _df,_f
end
function ∂f_res_disp(model,V,T,z)
    f(∂V,∂T) = eos_res_disp(model,∂V,∂T,z)
    _f,_df = Solvers.fgradf2(f,V,T)
    return _df,_f
end
...
"""
f_hess_res (mode,V,T,z)

returs the second order volume ('V') and temperature ('T') derivatives of the residual helmholtz energy

"""
function f_hess_res(model,V,T,z)
    f(w) = eos_res(model,first(w),last(w),z)
    V,T = promote(V,T)
    VT_vec = SVector(V,T)
    return Solvers.hessian(f,VT_vec)
end
...
"""
f_hess_res_hs(model,V,T,z)
f_hess_res_chain(model,V,T,z)
f_hess_res_disp(model,V,T,z)

returs the second order volume ('V') and temperature ('T') derivatives of the residual helmholtz energy respect to hs, hc, and dispersive terms 
of helmholtz contribution

"""

function f_hess_res_hs(model,V,T,z)
    f(w) = eos_res_hs(model,first(w),last(w),z)
    V,T = promote(V,T)
    VT_vec = SVector(V,T)
    return Solvers.hessian(f,VT_vec)
end
function f_hess_res_chain(model,V,T,z)
    f(w) = eos_res_chain(model,first(w),last(w),z)
    V,T = promote(V,T)
    VT_vec = SVector(V,T)
    return Solvers.hessian(f,VT_vec)
end
function f_hess_res_disp(model,V,T,z)
    f(w) = eos_res_disp(model,first(w),last(w),z)
    V,T = promote(V,T)
    VT_vec = SVector(V,T)
    return Solvers.hessian(f,VT_vec)
end
