...
"""
For the PC-SAFT and SAFTgamma Mie I added three residual function, hs,hc,disp in order to take into account the derivvative of 
residual helmholtz energy (hs,hc,disp) seperatly

"""
function eos_res_hs(model::EoSModel, V, T, z=SA[1.0])
    return Rgas(model)*sum(z)*T*a_res_hs(model,V,T,z)
end
function eos_res_chain(model::EoSModel, V, T, z=SA[1.0])
    return Rgas(model)*sum(z)*T*a_res_chain(model,V,T,z)
end
function eos_res_disp(model::EoSModel, V, T, z=SA[1.0])
    return Rgas(model)*sum(z)*T*a_res_disp(model,V,T,z)
end
